"""Entity that plays a `ASMACAG.Game.Game.Game` by using the N-Tuple Bandit Evolutionary Algorithm to model fitness and
 evolve a list of `ASMACAG.Game.Action.Action` based on it, composing a turn."""
import copy
import math
import random
import time
...
class NTBEAPlayer:
    def __init__(self, heuristic: "ASMACAG.Heuristics.Heuristic.Heuristic", dimensions: list[int], c_value: float,
                 neighbours: int, mutation_rate: float, initializations: int):
        """Entity that plays a `ASMACAG.Game.Game.Game` by using the N-Tuple Bandit Evolutionary Algorithm to model
        fitness and evolve a list of `ASMACAG.Game.Action.Action` based on it, composing a turn."""
...
# region Methods
    def think(self, observation: "ASMACAG.Game.Observation.Observation", budget: float) -> "ASMACAG.Game.Action.Action":
        """Computes a list of `ASMACAG.Game.Action.Action` for a complete turn using the N-Tuple Bandit Evolutionary
    ...
    def compute_turn(self, observation: "ASMACAG.Game.Observation.Observation", budget: float, initializations: int)\
            -> None:
        """Computes a list of `ASMACAG.Game.Action.Action` for a complete turn using the N-Tuple Bandit Evolutionary
        Algorithm  it as the turn."""
        t0 = time.time()
    ...
    def valid_initialization(self, observation: "ASMACAG.Game.Observation.Observation", initializations: int) \
            -> "Tuple[list[int], float]":
        """Generates a given amount of complete valid turns randomly and adds their stats to the bandit-based model,
        returning the best turn found and the score it yielded."""
        population = []
    ...
    def get_random_individual_valid(self, observation: "ASMACAG.Game.Observation.Observation") -> list[int]:
        """Generates a random turn that is valid for the given observation. Note that the observation state after
        running this method will be the result of playing the turn."""
        individual = []
        for i in range(self.dimension_amount):
    ...
    def get_neighbours(self, individual: list[int], neighbour_amount: int, mutation_rate: float) -> list[list[int]]:
        """Generates a list of neighbours from an individual. It changes at least one parameter (randomly chosen).
        The rest of them can change depending on the mutation rate."""
        population = []
    ...
    def get_total_ucb(self, individual: list[int]) -> float:
        """Returns the UCB of an individual, being the mean of its UCB for each bandit. If the individual is not
        in a bandit it will return a big number."""
        acm = 0
        # 1D
